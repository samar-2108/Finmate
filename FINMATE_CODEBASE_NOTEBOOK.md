...
**Problem:** `api.js` defines `sendChat` as:
```js
export async function sendChat(message, overrideUser = null, overrideQuiz = null)
```
But `Chat.jsx` calls it as:
```js
await sendChat(userProfile, quizAnswers, greetingPrompt, [], experiencePct)
// and:
await sendChat(userProfile, quizAnswers, text, apiHistory, experiencePct)
```

The argument positions are completely wrong. `message` gets `userProfile` (an object), `overrideUser` gets `quizAnswers` (an array), etc. The actual text message is in position 3, not position 1. This means the API receives garbage data.
...
Then update **both** `sendChat` calls in `Chat.jsx` to match the correct signature:
```js
// Greeting (Chat.jsx ~line 110):
const res = await sendChat(greetingPrompt, userProfile, quizAnswers, [], experiencePct);

// Regular message (Chat.jsx ~line 155):
const res = await sendChat(text, userProfile, quizAnswers, apiHistory, experiencePct);
```

---

### BUG #3 — HIGH: Stripped persona object after onboarding
**File:** `OnboardingForm.jsx` (finish function), `AuthContext.jsx` (saveProfile)

**Problem:** After saving the profile, `onComplete` is called with:
```js
onComplete(financialData, quizAnswers, { type: result.persona_type })
```
This passes only `{ type: "FIRE Seeker" }` as the persona. But `Chat.jsx`'s sidebar needs `persona.risk_score`, `persona.risk_profile`, `persona.emoji_label`, etc. These are `undefined` on first load, causing the persona card to render incorrectly.

**Fix:** After `apiSaveProfile()`, immediately call `apiGetMe()` to fetch the full persona from the DB, then pass the full object to `onComplete`:
```js
// OnboardingForm.jsx — finish()
const saveResult = await apiSaveProfile(financialData, quizAnswers, expPct);
const meData = await apiGetMe();  // fetch full persona from DB
onComplete(financialData, quizAnswers, meData.persona || { type: saveResult.persona_type });
```

---

### BUG #4 — MEDIUM: `AuthContext` import path mismatch
**File:** `App.jsx`, `AuthPage.jsx`, `OnboardingForm.jsx`, `Chat.jsx`

**Problem:** `App.jsx` imports from `"./contexts/AuthContext"` but the file listing shows `AuthContext.jsx` in the project root (not inside a `contexts/` subfolder). The project structure in the listing doesn't show a `contexts/` directory.

**Fix:** Either:
- Create the directory `src/contexts/` and move `AuthContext.jsx` there, OR
- Fix imports to `"../AuthContext"` (if files are in `src/components/`)

The exact fix depends on the actual folder structure — the file listing doesn't clarify subdirectory layout.

---

### BUG #5 — MEDIUM: `calc_fire_date()` uses wrong inflation for first year
**File:** `financial_engine.py`

**Problem:** In `run_calculations()` in `main.py`:
```python
raw_inflation = market.get("cpi_inflation", 6.0)
inflation = raw_inflation / 100 if raw_inflation > 1 else raw_inflation
```
`cpi_inflation` from the market snapshot is stored as `5.10` (a percentage), so `inflation` becomes `0.051`. This is correct. BUT in `calc_fire_corpus()`:
```python
inflation = KNOWLEDGE_BASE["inflation"]["general_cpi"]  # = 0.06
```
The corpus calculation uses a hardcoded 6% while the FIRE date simulation uses live CPI. These can produce a mismatch where the target corpus is computed at a different inflation rate than the yearly expense growth.

**Fix:** Pass the resolved `inflation` value explicitly to both functions in `run_calculations()`:
```python
fire = calc_fire_date(..., inflation=inflation)
fire_corpus = calc_fire_corpus(user_dict["monthly_expenses"], inflation=inflation)
```

---

### BUG #6 — MEDIUM: Missing `goals` selection in onboarding
**File:** `OnboardingForm.jsx`

**Problem:** The `form` state initialises `goals: ["retirement"]` but there is no UI in any of the 5 steps to let the user choose or change their goals. The system prompt explicitly mentions goals, and the sidebar shows them — but users are stuck with `["retirement"]` forever.

**Fix:** Add a goals multi-select (checkboxes) to step 0 or step 4 with options like: retirement, child_education, home_purchase, travel, child_marriage, car_purchase.

---

### BUG #7 — LOW: `experience_spend_pct` not sent in regular chat calls
**File:** `Chat.jsx`

**Problem:** `experiencePct` (default 20) is captured in the Chat component props but the `sendChat` calls use it only if the `api.js` function actually includes it in the request body. Given Bug #2 above, this is currently not working at all. Even after fixing Bug #2, the `experience_spend_pct` should be passed on every chat call so the backend persona detection uses it.

**Fix:** Addressed by the Bug #2 fix (passing `experiencePct` as the 5th argument after the fix).

---

### BUG #8 — LOW: `appendChatTurn()` in AuthContext never called
**File:** `AuthContext.jsx`, `Chat.jsx`

**Problem:** `AuthContext` exports `appendChatTurn()` for keeping local chatHistory in sync. But `Chat.jsx` manages its own `messages` and `apiHistory` state independently and never calls `appendChatTurn()`. So `chatHistory` in AuthContext stays as the DB-loaded initial history and doesn't reflect the current session's messages.
